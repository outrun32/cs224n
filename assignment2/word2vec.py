...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    #Calculate predicted y_hat -> array of shape (num_words, )
    y_hat = softmax(outsideVectors @ centerWordVec)
    #Create true distribution y -> array of shape(num_words, )
    y = np.zeros_like(y_hat)
    y[outsideWordIdx] = 1

    loss = -np.log(y_hat[outsideWordIdx])
    gradCenterVec = outsideVectors.T @ (y_hat - y)
    gradOutsideVecs = np.outer((y_hat-y), centerWordVec) #possible change
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    #calculate the gradient for outside word
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        ...
        loss_w2v, gradCenterVec, gradOutsideVecs = word2vecLossAndGradient(centerWordVec=center_word_vec, outsideWordIdx=word_ind, outsideVectors=outsideVectors, dataset=dataset)
        loss += loss_w2v
        gradCenterVecs[center_word_ind] += gradCenterVec
